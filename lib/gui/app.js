...
app.run(($timeout) => {
  ...
  driveScanner.on('error', (error) => {
    ...
    // TODO: Continue scanning once drivelist is fixed (see below)
    driveScanner.stop();
    ...
    // TODO: Once drivelist can recover from transient failures
    // (i.e. when a device is unplugged while it's scanning),
    // only send `analytics.logException(error);` and continue scanning
    if (!driveScanner.erroCount) {
